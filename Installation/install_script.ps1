...
Invoke-Sqlcmd -InputFile "$Path\_ATLAS_\Queries\Restore Database\AtlasDB.sql" -ServerInstance $Server -Username $User -Password $Password -QueryTimeout 120 ;
Write-Host "Finish.`r`n" ; 

Write-Host "Database Log Restored..." ;
Invoke-Sqlcmd -InputFile "$Path\_ATLAS_\Queries\Restore Database\LogDB.sql" -ServerInstance $Server -Username $User -Password $Password -QueryTimeout 120 ;
Write-Host "Finish.`r`n" ; 

### Assembly Database #############################################################################

Write-Host "Database Assembly Attached..." ;
Invoke-Sqlcmd -InputFile "$Path\_ATLAS_\Assembly\AddAssembly.sql" -ServerInstance $Server -Username $User -Password $Password -QueryTimeout 120 ;
Write-Host "Finish.`r`n" ; 
...
New-Item "IIS:\Sites\ATLAS" -bindings @{protocol="http";bindingInformation=":80:"} -physicalPath "$Path\_ATLAS_\ATLAS" -Force | Out-Null ;
Write-Host "Finish.`r`n" ;
...
New-Item "IIS:\Sites\GTSWS" -bindings @{protocol="http";bindingInformation=":81:"} -physicalPath "$Path\_ATLAS_\GTSWS" -Force | Out-Null ;
Write-Host "Finish.`r`n" ;
